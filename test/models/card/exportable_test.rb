...
class Card::ExportableTest < ActiveSupport::TestCase
  test "export_json returns card data as JSON" do
    ...
    assert_equal projects(:website_redesign).id, json["project"]["id"]
    assert_equal "Website redesign", json["project"]["name"]
    assert_equal "2026-08-28", json["project"]["due_on"]
    assert_equal milestones(:design_signoff).id, json["milestone"]["id"]
    assert_equal "Design sign-off", json["milestone"]["name"]
    assert_equal "2026-08-24", json["milestone"]["due_on"]
